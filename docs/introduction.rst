...
The xaitk-saliency API consists of a number of object-oriented interfaces for saliency heatmap generation, which
focus on black-box visual saliency. Three types of implementations of these interfaces are provided: Image
Perturbation, Heatmap Generation, and End-to-End Saliency Generation.


Image Perturbation
^^^^^^^^^^^^^^^^^^^
These implementations are occlusion-based algorithms that perform perturbations on image pixels.

* RISEGrid - Transforms an input reference image into a number of grid-based mask matrices indicating the perturbed regions.
  See the "DRISE" Jupyter Notebook example.

* SlidingRadial - Produces perturbation matrices generated by sliding a radial occlusion area with configured radius
  over the area of an image. When the two radius values are the same, circular masks are generated; otherwise,
  elliptical masks are generated. See the "Radial_Image_Perturbation" and "atari_deepRL_saliency"
  Jupyter Notebook examples.

* SlidingWindow - Produces perturbation matrices based on hard, block-y occlusion areas as generated by sliding a
  window of a configured size over the area of an image. See the "SimilarityScoring" Jupyter Notebook example.


Heatmap Generation
^^^^^^^^^^^^^^^^^^^
These implementations transform black-box image classification scores into saliency heatmaps.

* DRISEScoring - Transforms black-box object detector predictions into visual saliency heatmaps.
  See the "DRISE" Jupyter Notebook example.

* OcclusionScoring - Transforms black-box image classification scores into saliency heatmaps.
  This should require a sequence of per-class confidences predicted on the reference image, a number of per-class
  confidences as predicted on perturbed images, as well as the masks of the reference image perturbations.
  See the "SuperPixelSaliency" Jupyter Notebook example (also used internally in the "OcclusionSaliency" notebook).

* RISEScoring - Generates saliency maps that are based on the original RISE implementation,
  utilizing only the input perturbed image confidence predictions and not the reference image confidences
  (also used internally in the "OcclusionSaliency" notebook).

* SimilarityScoring - Transforms proximity in feature space into saliency heatmaps.
  See the "SimilarityScoring" Jupyter Notebook example.


End-to-End Saliency Generation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
These are the combined stack implementations that use a black-box classifier as input along
with a reference image to generate visual saliency heatmaps.

* PerturbationOcclusion - Uses modular perturbation and occlusion-based algorithms to generate saliency maps.

* RISEStack - Encapsulates the perturbation-occlusion method using the RISE implementations of the
  component algorithms. See the "covid_classification", "OcclusionSaliency", and "VIAME_OcclusionSaliency"
  Jupyter Notebook examples.

* SlidingWindowStack - Encapsulates the perturbation-occlusion method using sliding windows and the
  occlusion-scoring method. See the "covid_classification", "OcclusionSaliency", "MNIST_scikit_saliency", and
  "VIAME_OcclusionSaliency" Jupyter Notebook examples.
