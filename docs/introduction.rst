...
Image Perturbation
^^^^^^^^^^^^^^^^^^^
These implementations are occlusion-based saliency algorithms that perform perturbations on image pixels.

* RISEGrid - Transforms an input reference image into a number of mask matrices indicating the perturbed regions and
  generates an importance map indicating how salient each pixel is for the model's prediction.
...
* SlidingWindow - Produces perturbation matrices based on hard, block-y occlusion areas as generated by sliding a
  window of a configured sizse over the area of an image. See the "SimilarityScoring" Jupyter Notebook example.
...
* OcculsionScoring - Transforms black-box image classification scores into saliency heatmaps.
  ...
  See the "SuperPixelSaliency" Jupyter Notebook example.
