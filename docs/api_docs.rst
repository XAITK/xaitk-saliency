...
Method: ``perturb``  (alias: ``__call__``)

This method transforms an input reference image into a number of mask matrices indicating algorithm-determined regions of perturbation. The input image should be in the form of a `numpy.ndarray` matrix with a shape of either [H x W] or [H x W x C].

Output mask matrix should be 3-dimensional with the format [nMasks x Height x Width], sharing the same height and width to the input reference image matrix height and width dimensions. The implementing algorithm may determine the quantity of output masks per input image, as well as the data-type of the matrix (bool, uint8, float32, etc.). These masks should indicate regions in the reference image that the algorithm has determined should be modified for black-box input. Mask values should be in the [0, 1] range, where a value closer to 1.0 indicates areas of the image that are unperturbed. Note that output mask matrices may be of a floating-point type in order to allow for fractional perturbation.
...
Method: ``generate``  (alias: ``__call__``)

Input

* First image reference feature-vector (numpy.ndarray, shape [nFeats], float)

* Second image reference feature-vector (numpy.ndarray, shape [nFeats], float)

* Perturbed image feature-vectors, second reference image (numpy.ndarray, shape [nMasks x nFeats], float)

* Perturbation masks, second reference image (numpy.ndarray, shape [nMasks x H x W], float, [0,1] range)
...
* Reference image predicted class-confidence vector (numpy.ndarray, shape [nClasses], float, [0,1] range)

* Perturbed image predicted class confidences matrix (numpy.ndarray, shape [nMasks x nClasses], float, [0,1] range)

* Perturbation masks (numpy.ndarray, shape [nMasks x H x W], float, [0,1] range)
...
* Reference image object detections (numpy.ndarray, shape [nDets x (4 + 1 + nClasses)], float)

* Perturbed image object detections (numpy.ndarray, shape [nMasks x nProps x (4 + 1 + nClasses)], float)

* Perturbation masks (numpy.ndarray, shape [nMasks x H x W], float, [0,1] range)

Output

* Generated saliency heat-map matrix (numpy.ndarray, shape [nDets x H x W], float)
