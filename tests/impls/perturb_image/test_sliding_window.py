...
class TestOcclusionBasedPerturb:
    ...
    def test_perturb_3channel_nonsquare(self, snapshot_custom: SnapshotAssertion) -> None:
        """Test basic perturbation on a known image with non-square window + stride.
    ...
    def test_window_size_agnostic(self) -> None:
        """Test that the number of masks remains the same, with a given stride, as window size changes."""
        img = np.empty((21, 21))
